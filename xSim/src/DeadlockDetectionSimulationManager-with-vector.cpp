...
BeginWhile:
			...
			while(localSimulationTime < simulateUntil)
			{
				...
				else if(nextEvent == null && cluster->workingInputChannelCount == 0)
					goto EndWhile;
...
EndWhile:
        ...
        EventInterface *DeadlockDetectionSimulationManager::GetNextEvent(LogicalProcess *lp)
        {
            ...
            if(cluster->messagesIndex == cluster->messages->Size() || cluster->messages->Size() == 0)
                return null;
            ...
            {
                //mtx->Lock(); std::cout << "p.second " << p.second << " cluster->GetInputChannelByIndex(p.second) " << cluster->GetInputChannelByIndex(p.second) << std::endl; mtx->Unlock();
                cluster->GetInputChannelByIndex(p.second)->Dequeue();
            }

            cluster->messagesIndex++;
            cluster->previous_event = p.first;
            ...
            /// (message, time_stamp) = receive();
            /// time = max(time_stamp, time) + 1;
            //if(p.first->GetReceiveTime() != p.first->GetSendTime())
            //    cluster->lc_time = 0;
            //else
            cluster->lc_time = max(cluster->lc_time, p.first->lc) + 1;
            //if(cluster->GetClusterID() == 0)
            //std::cout << "ClusterID " << cluster->GetClusterID() << " cluster->lc_time " << cluster->lc_time << std::endl;
        ...
        EventInterface *DeadlockDetectionSimulationManager::PeekNextEvent(LogicalProcess *lp)
        {
            ...
            if(cluster->messagesIndex == cluster->messages->Size() || cluster->messages->Size() == 0)
            {
                ...
                if(nextEvent != null && nextEvent->GetReceiveTime() == cluster->GetLocaltime())
                {
                    //for(cluster->messagesIndex = 0 ; cluster->messagesIndex < cluster->messages->Size() ; cluster->messagesIndex++)
                    //{
                    //    std::pair<EventInterface *, Int32> pp = (*cluster->messages)[cluster->messagesIndex];
                    //    EventInterface *e = pp.first;
                    //    if(e->eventType == NormalMessage)
                    //    {
                    //       if(e->deletable)
                    //       {
                    //            LogicalProcess *r = LogicalProcess::GetLogicalProcessById(e->GetSenderReference());
                    //            r->DeallocateEvent(e);
                    //        }
                    //    }
                    //}
                    ...
                    receiver->SetSimulationTime(cluster->GetLocaltime());
                    std::pair<EventInterface *, Int32> pp;
                    pp.first = nextEvent;
                    pp.second = -1;
                    cluster->messages->PushBack(pp);
                    return nextEvent;
                }
                else
                    return null;
            ...
            std::pair<EventInterface *, Int32> ppp = (*cluster->messages)[cluster->messagesIndex];

            //return ppp.first;
            if(ppp.first->GetEventType() == NormalMessage || ppp.first->GetEventType() == AppNullMessage)
                return ppp.first;
            else
                return null;
        }
        ...
        void DeadlockDetectionSimulationManager::SendEvent(EventInterface *e)
        {
            ...
            //if(e->GetReceiveTime() != e->GetSendTime())
            //    srcCluster->lc_time = 0;
            //else
            srcCluster->lc_time = srcCluster->lc_time + 1;
            e->lc = srcCluster->lc_time;
            //e->lc_cluster = srcClusterID;///

            //srcCluster->nonLocalSentMessages++;

            // Send null messages
            /*if(e->GetEventType() == NullMessage)
            {
                //for(register Int32 dest = srcCluster->_destinationClusters.Size() - 1 ; dest != -1 ; dest--)
                for(register UInt32 i = 0 ; i < srcCluster->GetDestinationClusterCount() ; i++)
				{
					//DeadlockDetectionPriorityQueue *destPQ = srcCluster->_destinationClusters[dest];
					//mtx->Lock();std::cout << "Cluster " << srcClusterID << " send null message to Cluster " << destPQ->GetOwnerClusterID() << endl; mtx->Unlock();///
					DeadlockDetectionPriorityQueue *destPQ = srcCluster->GetDestinationClusterChannelByID(i);
					if(destPQ != null)
					{
                        EventInterface *newEvent = new EventInterface();
                        newEvent->SetEventType(NullMessage);
                        newEvent->SetSenderReference(senderID);
                        newEvent->SetSendTime(e->GetSendTime());
                        newEvent->SetReceiveTime(e->GetReceiveTime());
                        //destPQ->Enqueue(newEvent); // if dest is empty, send a null-message
                        destPQ->Enqueue(newEvent);
                        srcCluster->nullMessageCounter++;
					}
            ...
            else*/
            {
                ...
                else
                {
                    ...
                    //if(srcCluster->GetDestinationClusterCount() > 1)
                    //	SendNullMassage(srcClusterID, destClusterID);
                }
            }
        }
		//----------------------------------------------------
